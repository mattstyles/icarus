...
module.exports = function( req, res ) {
    ...
    // Set up the socket
    if ( icarus.socket ) {
        icarus.log.info( 'Connected via socket' );

        // Create socket event listeners
        icarus.socket.on( 'deploytest', function( data ) {
            console.log( 'received some shizzle via the socket' );
        });
    }
    ...
    var npmInstall = function( cb ) {
        ...
        var child = exec( 'npm install', { cwd: deployPath }, function( err, stdout, stderr ) {
            ...
            icarus.socket.emit( 'deployed', { data: 'passing data example' } );
