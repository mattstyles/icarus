...
module.exports = {
     ...
     * input functions
     *
     * messy, should be moved to io module
     */
    ...
    getConfirm: function( str, success, failure ) {
        // Add y/n to the prompt message
        str = str + ' (y/n) '.white;

        //  Prompt and fire success or failure based on response
        icarus.prompt( str, function( res ) {
            if ( res[str].toLowerCase() === 'y' || res[str].toLowerCase() === 'yes' ) {
                success();
            } else {
                failure();
            }
