...
module.exports = function( opts ) {
    icarus.out( 'Message from the test function' );
    icarus.out( 'finding package...');

    var find = function( pwd, done ) {
        console.log( pwd );

        fs.exists( path.join ( pwd, 'package.json' ), function( file ) {
            if ( (file || process.cwd() === process.env.HOME) && !pwd.match(/node_modules/) ) {
                console.log( 'package found or hit the home directory' );

                done( pwd );
            } else {
                console.log( 'recursing...' );
                find( path.join( pwd, '..' ), done );
            }
        });
    }
...
//    });
    ...
    icarus.utils.findProjectRoot( function( found ) {
        icarus.out( found ? 'package found' : 'package not found' );
    });

//    icarus.utils.findProjectRoot( function( dir ) {
//        console.log( 'Error in finding project root at ' + dir );
//    }, function( dir ) {
//        console.log( 'Found the project root at ' + dir );
//    });
