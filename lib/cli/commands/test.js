...
module.exports = function( opts ) {
    ...
    socket.on( 'connect', function() {
        console.log( 'cli connected to server via sockets' );
    });

    // Handle news event.
    // Send back a reaction to the news event - use broadcast for public (not applicable at the moment)
    socket.on( 'news', function( data ) {
        console.log( 'receiving news event from server: ' + data.data );

        socket.emit( 'reaction', {
            data: 'That was some amazing news man'
        });
    });

    // Handle morenews event.
    socket.on( 'morenews', function( data ) {
        console.log( 'Following a swift reaction here is even more news: ' + data.data );

        // The socket code will keep the connection open and the cli program running so manually exit
        process.exit(0);
    });
}