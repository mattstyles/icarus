...
module.exports = (function() {
    ...
    var deployFromGit = function() {
        ...
        var archive = path.join( tmpFolder, archiveName );

        // Build query string
        query = {
            pkgName: icarus.utils.parsePkgName( pkg.name ) || 'not specified',
            tmpName: archive || 'not specified',
            build: false
        };
        ...
        getAuth( function( token ) {
            ...
            function onGetRepo( err, res, body ) {
                if ( err ) {
                    icarus.log.error( 'Error connecting to github: '.error + err );
                }
...
//                        icarus.log.debug( 'Repository found'.info );
                        break;
            ...
            // @todo parse github url properly from the info in the package
            var getRepoRequest = {
                url: 'repo',
                qs: {
                    access_token: token
...
//                    console.log( res.statusCode );
                ...
                .pipe( request.post( postRepoRequest, onPostRepo ) )
                    .on( 'error', function( err ) {
                        console.log( 'error from the pipe' );
                    });
