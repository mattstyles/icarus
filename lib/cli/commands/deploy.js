...
module.exports = (function() {
    ...
    var onProjectFolder = function( dir ) {
        ...
        async.parallel( {
            pkgtest: async.apply( icarus.utils.getPkg, dir ),
            conf: icarus.utils.getConf
        }, function( err, res ) {
            if ( err ) {
            ...
            // Pass the projectr root directory to the options router
            optsRouter( dir );
        } );
    };
    ...
    var sendToRemote = function( tmpPath, pkg ) {
        ...
        // Don't .end the request as we are piping it
        var req = request
                    .post( route.protocol + route.host + ':' + route.port + '/' + route.path )
                    .type( 'binary' )
                    .query( query )             // @todo as its a post should this be a send rather than a query?
                    .on( 'error', function( err ) {
