...
module.exports = {
     ...
     * Parameters are optional so an arity of 0-3 is possible although an arity of 1 is strongly recommended with
     * the param being the callback function when the file is found.
     */
    findProjectRoot: function( dir, err, cb ) {
        ...
        fs.exists( path.join( dir, 'package.json' ), function( found ) {
            ...
            if ( dir === process.HOME || dir === '/' ) {
                // Log the error
                icarus.log.error( 'Error finding project root -- no file found' );
                icarus.log.debug( 'Current work directory: ' + process.cwd() );
                if ( dir === process.HOME ) {
                    icarus.log.debug( 'Hit HOME directory' );
                }
                if ( dir === '/' ) {
                    icarus.log.debug( 'Hit ROOT directory' );
                }
            ...
            if ( found && !(dir.match( exclude )) ) {
                ...
                icarus.out( 'Project Root: '.info + path.resolve( dir ) );
